...
interface RGBBackgroundProps {
    matrixMode?: boolean;
}

export const RGBBackground: React.FC<RGBBackgroundProps> = ({ matrixMode = false }) => {
  return (
    <div className="fixed inset-0 z-0 pointer-events-none bg-[#09090b]">
       ...
       {matrixMode && (
           <div className="absolute inset-0 bg-[url('https://www.transparenttextures.com/patterns/matrix-code.png')] opacity-20 animate-pulse pointer-events-none mix-blend-screen"></div>
       )}
    </div>
  );
};